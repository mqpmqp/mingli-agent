...
当前基线：`main@40bab90dde185ff97fa983f5a50ce7b8208cb11e`（PR A #28 merge commit）
...
- RED `00cb96d`：复现额外组合规则与重复宫名 false pass，目标结果 2 failed / 9 passed。
- RED `cef9774`：复现主星证据 0/14 有效、非法修饰星与重复宫序，目标结果 4 failed / 9 passed。
- GREEN `819e409`：冲突键收窄到同一 domain/subject/触发目标；命盘先过完整 Schema，再校验十二宫、二十八星、四化和修饰星归属；184 条规则逐类行为求值，受影响目标 14 passed。
...
- RED `7c189f6`：终审三项边界共 6 failed。
- RED `da20289`：exclusions mutation 1 failed。
- GREEN `e864f71`：冲突键改为规范化 trigger；所有嵌套记录必须 `supported`；coverage 要求 canonical trigger 和必需 exclusions，并执行正样本、空目标、degraded、unsupported 四类行为求值；定向 7 passed，规则内容文件 45 passed。
- 测试闭环 `4448d51`：将 primary/supporting/malefic/transformation/brightness × unsupported/research_required 的 10 个组合全部固化，10 passed。
...
- PR A #28 已合并；PR B 已 restack 到 `main@40bab90`，本地全门禁复验通过后进入 Draft PR 审查。
