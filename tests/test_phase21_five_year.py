...
class Phase21Tests(unittest.TestCase):
    ...
    def test_exact_window_and_renderer_adapter(self):
        ...
        self.assertTrue(all(year.confidence == "low" for year in result.years))
        self.assertTrue(all(set(year.domain_confidence.values()) == {"low"} for year in result.years))
    def test_conflicting_verified_reality_unresolved(self):
        ...
        self.assertEqual("low", result.years[2].domain_confidence["career"])
    def test_verified_reality_is_high_only_for_its_scoped_domain(self):
        raw = {"anchor_year": 2026, "baseline_domains": {}, "annual_evidence": [{"evidence_id": "x", "year": 2026, "domain": "career", "signal": 0, "verified_reality": "support", "source_type": "reality", "source_id": "verified", "verified": True}]}
        year = generate_five_year_outlook(raw).years[2]
        self.assertEqual("high", year.domain_confidence["career"])
        self.assertEqual("low", year.confidence)
