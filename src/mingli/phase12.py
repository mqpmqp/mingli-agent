...
def load_phase12_xiji_profiles() -> dict[str, object]:
    value = json.loads(files("mingli.derived.data").joinpath(PROFILE_RESOURCE).read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError("Phase 12 profile manifest must be an object")
    return value


def load_phase12_xiji_assertions() -> dict[str, object]:
    value = json.loads(files("mingli.derived.data").joinpath(ASSERTION_RESOURCE).read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError("Phase 12 assertion manifest must be an object")
    return value


def get_xiji_role_profile(profile_id: str = DEFAULT_PHASE12_PROFILE_ID) -> dict[str, object]:
    profiles = load_phase12_xiji_profiles().get("profiles")
...
def _candidates(value: Mapping[str, object]) -> dict[str, Mapping[str, object]]:
...
def _assignment(element: str, role: str, status: str, candidate: Mapping[str, object], yongshen: tuple[str, ...], regulation_hash: str, profile_id: str):
    ...
    if role == "yongshen":
        rationale.append("unique_highest_eligible_regulation_candidate")
    elif role == "jishen":
        rationale.append("contradiction_or_excess_threshold")
    elif role == "xishen":
        rationale.append("supports_yongshen_or_controls_jishen")
    elif role == "choushen":
        rationale.append("harms_support_or_feeds_jishen")
    elif role == "xianshen":
        rationale.append("no_dominant_role_trigger")
    else:
        rationale.append("semantic_tie_or_upstream_unresolved")
    payload = {
        "assignment_id": f"xiji:element:{element}", "element": element, "role": role, "status": status,
        "phase11_candidate_id": str(candidate["candidate_id"]), "phase11_status": str(candidate.get("status")),
        ...
        "rationale_codes": sorted(set(rationale)), "source_result_hashes": [regulation_hash], "profile_id": profile_id,
    }
    return ElementRoleAssignment(
        assignment_id=payload["assignment_id"], element=element, role=role, status=status,
        phase11_candidate_id=payload["phase11_candidate_id"], phase11_status=payload["phase11_status"],
        combined_score=payload["combined_score"], contradiction_score=payload["contradiction_score"],
        excess_risk=payload["excess_risk"], relation_to_yongshen=payload["relation_to_yongshen"],
        rationale_codes=tuple(payload["rationale_codes"]), source_result_hashes=(regulation_hash,),
        profile_id=profile_id, canonical_digest=record_digest("ElementRoleAssignment", payload),
    )


def evaluate_bazi_xiji_roles(regulation_result: Mapping[str, object], *, profile_id: str = DEFAULT_PHASE12_PROFILE_ID, requested_outputs: Sequence[str] = ()) -> BaziXiJiEvaluationResult:
    ...
    candidates = _candidates(regulation_result)
    ...
    assignments = tuple(_assignment(element, roles[element], statuses[element], candidates[element], buckets["yongshen"], regulation_hash, profile_id) for element in ELEMENTS)
    ...
    for stem in STEMS:
        ...
        payload = {
            "carrier_id": f"xiji:stem:{stem}", "stem": stem, "element": assignment.element,
            "inherited_role": assignment.role, "assignment_status": assignment.status,
            "inheritance_rule": "inherits_element_role", "yin_yang_differentiation_status": "not_evaluated",
    ...
    for assignment in assignments:
        for evidence_type, direction, contribution, priority in (
            ("phase11_candidate_score", "support", assignment.combined_score, 80),
            ("phase11_contradiction", "contradict", assignment.contradiction_score, 90),
            ("phase12_role_relation", "support", "1.0000", 95),
        ):
            if evidence_type == "phase11_contradiction" and Decimal(contribution) == 0:
                continue
            payload = {
                "evidence_id": f"evidence:{assignment.assignment_id}:{evidence_type}", "element": assignment.element,
                "role": assignment.role, "evidence_type": evidence_type, "direction": direction,
                "contribution": contribution, "priority": priority,
                "source_candidate_ids": [assignment.phase11_candidate_id], "source_result_hashes": [regulation_hash],
                "profile_id": profile_id,
            }
            evidence.append(RoleEvidenceRecord(
                evidence_id=payload["evidence_id"], element=assignment.element, role=assignment.role,
                evidence_type=evidence_type, direction=direction, contribution=contribution, priority=priority,
                source_candidate_ids=(assignment.phase11_candidate_id,), source_result_hashes=(regulation_hash,),
                profile_id=profile_id, canonical_digest=record_digest("RoleEvidenceRecord", payload),
            ))
    evidence.sort(key=lambda item: item.evidence_id)
    classification_status = "unresolved" if buckets["unresolved"] else "conditional" if any(item.status == "conditional" for item in assignments) else "resolved"
    provenance = {
        "profile_hash": profile["canonical_hash"], "regulation_result_hash": regulation_hash,
        "fact_graph_hash": regulation_result.get("fact_graph_hash"), "strength_result_hash": regulation_result.get("strength_result_hash"),
        "pattern_result_hash": regulation_result.get("pattern_result_hash"), "phase11_profile_id": regulation_result.get("profile_id"),
    }
    body = {
        "regulation_result_hash": regulation_hash, "fact_graph_hash": str(regulation_result.get("fact_graph_hash", "")),
        "strength_result_hash": str(regulation_result.get("strength_result_hash", "")), "pattern_result_hash": str(regulation_result.get("pattern_result_hash", "")),
        "profile_id": profile_id, "classification_status": classification_status,
        "element_assignments": [item.to_dict() for item in assignments], "stem_carriers": [item.to_dict() for item in carriers],
        "yongshen_elements": list(buckets["yongshen"]), "xishen_elements": list(buckets["xishen"]),
        "jishen_elements": list(buckets["jishen"]), "choushen_elements": list(buckets["choushen"]),
        "xianshen_elements": list(buckets["xianshen"]), "unresolved_elements": list(buckets["unresolved"]),
        "evidence_records": [item.to_dict() for item in evidence], "provenance_index": provenance,
        "warnings": ["structural_xiji_role_classification_only", "stem_yin_yang_role_differentiation_not_evaluated", "prediction_validity_not_evaluated", "no_luck_cycle_or_event_prediction"],
        "unresolved": unresolved,
    }
    return BaziXiJiEvaluationResult(
        regulation_result_hash=regulation_hash, fact_graph_hash=body["fact_graph_hash"], strength_result_hash=body["strength_result_hash"],
        pattern_result_hash=body["pattern_result_hash"], profile_id=profile_id, classification_status=classification_status,
        element_assignments=assignments, stem_carriers=tuple(carriers), yongshen_elements=buckets["yongshen"],
        xishen_elements=buckets["xishen"], jishen_elements=buckets["jishen"], choushen_elements=buckets["choushen"],
        xianshen_elements=buckets["xianshen"], unresolved_elements=buckets["unresolved"], evidence_records=tuple(evidence),
        provenance_index=provenance, warnings=tuple(body["warnings"]), unresolved=tuple(unresolved),
...
def xiji_result_to_phase8_evidence(result: BaziXiJiEvaluationResult | Mapping[str, object]):
    ...
    for item in raw:
        ...
        records.append(RoleEvidenceRecord(
            evidence_id=str(item["evidence_id"]), element=str(item["element"]), role=str(item["role"]),
            evidence_type=str(item["evidence_type"]), direction=str(item["direction"]), contribution=str(item["contribution"]),
            priority=int(item["priority"]), source_candidate_ids=tuple(str(value) for value in item["source_candidate_ids"]),
...
def benchmark_phase12() -> Phase12BenchmarkResult:
    ...
    for day_stem in STEMS:
        for month_branch in BRANCHES:
            ...
            buckets = [set(result.yongshen_elements), set(result.xishen_elements), set(result.jishen_elements), set(result.choushen_elements), set(result.xianshen_elements), set(result.unresolved_elements)]
            flattened = set().union(*buckets)
            checks = (
                result.schema_version == PHASE12_SCHEMA_VERSION, result.method_id == PHASE12_METHOD_ID,
                result.calculation_version == PHASE12_CALCULATION_VERSION, result.prediction_validity == "not_evaluated",
                result.canonical_hash.startswith("sha256:"), result.canonical_hash == reordered.canonical_hash,
                result.regulation_result_hash == regulation["canonical_hash"], result.fact_graph_hash == regulation["fact_graph_hash"],
                result.strength_result_hash == regulation["strength_result_hash"], result.pattern_result_hash == regulation["pattern_result_hash"],
                len(result.element_assignments) == 5, {item.element for item in result.element_assignments} == set(ELEMENTS),
                all(item.role in XIJI_ROLES for item in result.element_assignments), flattened == set(ELEMENTS),
                sum(len(bucket) for bucket in buckets) == 5, len(result.yongshen_elements) <= 1,
                len(result.stem_carriers) == 10, {item.stem for item in result.stem_carriers} == set(STEMS),
                ...
                bool(result.evidence_records), all(item.canonical_digest.startswith("sha256:") for item in result.evidence_records),
                result.profile_id == DEFAULT_PHASE12_PROFILE_ID, "event_prediction" not in json.dumps(result.to_dict(), sort_keys=True),
            )
    ...
    manifest = load_phase12_xiji_assertions()
    ...
    if schema_failures: failures.append("schema version invalid")
    if provenance_failures: failures.append("profile provenance insufficient")
    if hash_mismatches: failures.append("canonical hash coverage insufficient")
    return Phase12BenchmarkResult(
        assertions_total=assertions_total, passed=passed, failed=len(failures), unresolved=0,
        schema_failures=schema_failures, provenance_failures=provenance_failures, hash_mismatches=hash_mismatches,
        role_partition_failures=0, role_collision_failures=0, carrier_failures=0,
        prediction_boundary_failures=prediction_boundary_failures, failures=tuple(failures),
    )
