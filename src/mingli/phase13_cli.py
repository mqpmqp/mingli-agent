...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="mingli-phase13", description="Phase 13 deterministic Bazi luck-cycle role interaction tools")
    subcommands = parser.add_subparsers(dest="command", required=True)
    evaluate = subcommands.add_parser("evaluate")
    evaluate.add_argument("--graph", required=True, type=Path)
    evaluate.add_argument("--xiji", required=True, type=Path)
    evaluate.add_argument("--profile", default="luck-cycle-role-interaction-r1@0.1")
    ...
    try:
        ...
        if args.command == "benchmark":
            ...
            return 1 if any((
                ...
                result.timeline_failures,
                result.partition_failures,
                result.relation_failures,
                result.prediction_boundary_failures,
            )) else 0
        if args.command == "profiles":
            print(json.dumps(load_phase13_interaction_profiles(), ensure_ascii=False, sort_keys=True))
            return 0
        if args.command == "provenance":
            result = validate_import_origin(args.expected_root)
            print(json.dumps(result.to_dict(), ensure_ascii=False, sort_keys=True))
            return 0 if result.valid else 1
        print(json.dumps(phase13_schema_summary(), ensure_ascii=False, sort_keys=True))
        return 0
